...
from src.api.message.message_service import Users
...
def test_get_message_success(
    ...
    mock_message_repo.return_value.get_message_by_roomId.return_value = [
        Messages(id=1, room_member_id=1, room_id=123, user=Users(id=1),
                 created_at=datetime(2023, 12, 25, 15, 30, 0), message="hello"),
        Messages(id=2, room_member_id=1, room_id=123, user=Users(id=1),
                 created_at=datetime(2023, 12, 25, 15, 30, 0), message="world"),
        Messages(id=3, room_member_id=1, room_id=123, user=Users(id=1),
                 created_at=datetime(2023, 12, 25, 15, 30, 0), message="!"),
        Messages(id=4, room_member_id=1, room_id=123, user=Users(id=1),
                 created_at=datetime(2023, 12, 25, 15, 30, 0), message="I"),
        Messages(id=5, room_member_id=1, room_id=123, user=Users(id=1),
                 created_at=datetime(2023, 12, 25, 15, 30, 0), message="am"),
        Messages(id=6, room_member_id=1, room_id=123, user=Users(id=1),
                 created_at=datetime(2023, 12, 25, 15, 30, 0), message="here")
    ]
    ...
    # 預期的結果列表
    expected_messages = [
        Message(room_id=RoomId(id=123), message_id=MessageId(id=1), created_at=timestamp, author=UserId(id=1), text="hello"),
        Message(room_id=RoomId(id=123), message_id=MessageId(id=2), created_at=timestamp, author=UserId(id=1), text="world"),
        Message(room_id=RoomId(id=123), message_id=MessageId(id=3), created_at=timestamp, author=UserId(id=1), text="!"),
        Message(room_id=RoomId(id=123), message_id=MessageId(id=4), created_at=timestamp, author=UserId(id=1), text="I"),
        Message(room_id=RoomId(id=123), message_id=MessageId(id=5), created_at=timestamp, author=UserId(id=1), text="am"),
        Message(room_id=RoomId(id=123), message_id=MessageId(id=6), created_at=timestamp, author=UserId(id=1), text="here")
    ]

    # 用 for 迴圈來比較每一個 message 的屬性
    for expected_msg, actual_msg in zip(expected_messages, response.messages):
        assert expected_msg.room_id == actual_msg.room_id, f"RoomId mismatch: {expected_msg.room_id} != {actual_msg.room_id}"
        assert expected_msg.message_id == actual_msg.message_id, f"MessageId mismatch: {expected_msg.message_id} != {actual_msg.message_id}"
        assert expected_msg.created_at == actual_msg.created_at, f"CreatedAt mismatch: {expected_msg.created_at} != {actual_msg.created_at}"
        assert expected_msg.author == actual_msg.author, f"Author mismatch: {expected_msg.author} != {actual_msg.author}"
        assert expected_msg.text == actual_msg.text, f"Text mismatch: {expected_msg.text} != {actual_msg.text}"
