...
def init_db() -> None:
    cur.execute(
        "CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY, username TEXT, password TEXT, rooms TEXT)"
    )
    cur.execute(
        "CREATE TABLE IF NOT EXISTS rooms (id INTEGER PRIMARY KEY, room_id TEXT, name TEXT, username TEXT)"
    )
    cur.execute(
        "CREATE TABLE IF NOT EXISTS messages (id INTEGER PRIMARY KEY, target_username TEXT, msg_id TEXT, author_username TEXT, room_id TEXT, message TEXT, timestamp TEXT)"
    )
    con.commit()
...
def new_user(username: str, password: str) -> None:
    cur.execute(
        "INSERT INTO users (username, password, rooms) VALUES (?, ?, ?)",
...
def new_room(room_id: str, name: str, usernames: list[str]) -> None:
    cur.execute(
        "INSERT INTO rooms (room_id, name, username) VALUES (?, ?, ?)",
        (room_id, name, list_to_str(usernames)),
    )
    ...
    for username in usernames:
        ...
        cur.execute(
            "UPDATE users SET rooms = ? WHERE username = ?",
            (list_to_str(user_rooms), username),
        )
    con.commit()
...
def join_room(username: str, room_id: str) -> None:
    ...
    cur.execute(
        "UPDATE rooms SET username = ? WHERE room_id = ?",
        (list_to_str(room_users), room_id),
    )
    # commit
    con.commit()
...
def get_room(room_id: str) -> dict:
    cur.execute(
        "SELECT room_id, name, username FROM rooms WHERE room_id = ?", (room_id,)
    )
    result = cur.fetchone()
    return {
...
def new_message(
    author_username: str, room_id: str, message: str, timestamp: datetime
) -> None:
    # get room users
    cur.execute("SELECT username FROM rooms WHERE room_id = ?", (room_id,))
    result = cur.fetchone()
...
def get_user_messages(target_username: str) -> list:
    cur.execute(
        "SELECT msg_id, author_username, room_id, message, timestamp FROM messages WHERE target_username = ?",
        (target_username,),
    )
    result = cur.fetchall()
    messages = [
