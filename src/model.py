from sqlmodel import TEXT, String, SQLModel, Field, Relationship, BIGINT, Column, TIMESTAMP, func
...
class Users(SQLModel, table=True):
    __tablename__ = "users"
    id: int | None = Field(sa_column=Column(BIGINT,primary_key=True,default=generate_snowflake_id))
    ...
    rooms: list["Rooms"] = Relationship(
        ...
        sa_relationship_kwargs={"secondary": "room_members"}
    )
...
class Rooms(SQLModel, table=True):
    ...
    members: list['RoomMembers'] = Relationship(back_populates="room") # for custom misc
...
class RoomMembers(SQLModel, table=True):
    ...
    room_name: str = Field(sa_column=Column(String(256))) # user customize room name (show only user)
    room_avater_url: str = Field(sa_column=Column(TEXT)) # user customize room avater (show only user)
    user_name: str = Field(sa_column=Column(String(256)))  # custom user name in room (show all room user)
    user_avater_url: str = Field(sa_column=Column(TEXT))  # custom user avater in room (show all room user)
...
class Messages(SQLModel, table=True):
    ...
    id: int | None = Field(sa_column=Column(BIGINT,primary_key=True,default=generate_snowflake_id))
    room_member_id: int = Field(foreign_key="room_members.id",index=True,sa_type=BIGINT)
    room_id: int = Field(foreign_key="rooms.id",index=True,sa_type=BIGINT)
