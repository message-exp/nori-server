...
class RefreshTokenRepository:
    ...
    def update_refresh_token(self,user_id : int, old_refresh_token: str , new_refresh_token : str) -> None:
        # use old refresh token to find the RefreshToken in the database
        refresh_token_found = self.db.exec(
            select(RefreshToken).where(RefreshToken.user_id == user_id & RefreshToken.refresh_token == old_refresh_token)
        ).first()
