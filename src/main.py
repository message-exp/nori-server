...
class NoriServiceServicer(ServiceServicer):
    def CreateUser(
        self: Any, request: UserRequest, unused_context: ServicerContext
    ) -> Empty:
    ...
    def CreateRoom(self: Any, request: Room, unused_context: ServicerContext) -> Empty:
        logging.info(f"Create room {request.name}")
        room_id = db.generate_room_id()
        db.new_room(room_id, request.name, list(request.usernames))
        return Empty()

    def JoinRoom(
        self: Any, request: JoinRoomRequest, unused_context: ServicerContext
    ) -> Empty:
    ...
    def GetRoom(
        self: Any, request: RoomRequest, unused_context: ServicerContext
    ) -> Room:
        logging.info(f"Get room {request.room_id}")
        room = db.get_room(request.room_id)
        return Room(
            room_id=room["room_id"], name=room["name"], usernames=room["usernames"]
        )

    def SendMessage(
        self: Any, request: Message, unused_context: ServicerContext
    ) -> Empty:
        logging.info(
            f"Send message: user {request.author_id} in room {request.room_id} says {request.text}"
        )
        server_time = datetime.now(timezone.utc)
        user_timestamp = request.timestamp
        user_time = datetime.fromtimestamp(
            user_timestamp.seconds + user_timestamp.nanos / 1e9, timezone.utc
        )
    ...
    async def GetMessages(
        ...
        for message in messages:
            yield Message(
                ...
                timestamp=Timestamp(
                    seconds=int(message["timestamp"].timestamp()),
                    nanos=int(message["timestamp"].microsecond * 1e3),
                ),
            )
...
async def serve() -> None:
    ...
    add_ServiceServicer_to_server(NoriServiceServicer(), server)
