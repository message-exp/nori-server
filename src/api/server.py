...
def get_server() -> grpc.Server:
    ...
    add_RoomGeneralServiceServicer_to_server(
        servicer=RoomGeneralServicer(), server=server
    )
    add_RoomMemberServiceServicer_to_server(
        servicer=RoomMemberServicer(), server=server
    )
    add_UserAccessServiceServicer_to_server(
        servicer=UserAccessServicer(), server=server
    )
    add_UserNetworkServiceServicer_to_server(
        servicer=UserNetworkServicer(), server=server
    )
    add_UserAccountServiceServicer_to_server(
    ...
    SERVICE_NAMES = (
        ...
        user_access_service_pb2.DESCRIPTOR.services_by_name[
            "UserAccessService"
        ].full_name,
        user_account_service_pb2.DESCRIPTOR.services_by_name[
            "UserAccountService"
        ].full_name,
        user_network_service_pb2.DESCRIPTOR.services_by_name[
            "UserNetworkService"
        ].full_name,
        room_general_service_pb2.DESCRIPTOR.services_by_name[
            "RoomGeneralService"
        ].full_name,
        room_member_service_pb2.DESCRIPTOR.services_by_name[
            "RoomMemberService"
        ].full_name,
